...
def summarizer(pdf):
    ...
    if pdf is not None:
        ...
        4. En caso el documento sea una Ley o Normativa, y se encuentre divida por TITULOS, resume de forma separada cada Título.
        """
...
# --- FRONTEND ---
#######################################
st.set_page_config(page_title="The PDF Summarizer") # the "set_page_config()" must be on before all the fronted
# --- User Authentication ---
...
if st.session_state["authentication_status"]:
    with st.sidebar:
        ...
        st.markdown(" This app was made by *Mario Solis*")
    #st.title('Some content')

    def main():
        with st.sidebar:
            azure_api_key = st.text_input("Please, introduce your Azure API Key", type="password") 
        
        st.title("PDF Summarizing App")
        st.write("Summarize your PDF files in seconds")
        st.divider()

        pdf = st.file_uploader("Please, upload your PDF document", type="pdf")
